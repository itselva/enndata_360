...
import TableofEditTimeSheet from '../Components/TableofEditTimeSheet';
import { useNavigate } from 'react-router-dom';

const EditRejectTimesheet = () => {
      const [activeTab, setActiveTab] = useState("create");
  const [activeSubTab, setActiveSubTab] = useState("currentWeek");
  const [localIsTableDisabled, setLocalIsTableDisabled] = useState(true);
  const navigate = useNavigate();

  const handleWithdraw = () => {
    // Enable the table for editing
    
    setLocalIsTableDisabled(false);
    navigate("/RejectTimesheet");
  };
  return (
    <div>
         <Header />
                  ...
                  {activeTab === 'create' && (
                    <>
                      <div className="sub-tab-tabs">
                        ...
                        >
                          Not Submitted - <span className="number">5</span>
                        </div>
                      </div>
        
                      {(activeSubTab === "currentWeek" || activeSubTab === "notSubmitted") && (
                        <TableofEditTimeSheet isTableDisabled={localIsTableDisabled} />
                      )}
...
export default EditRejectTimesheet